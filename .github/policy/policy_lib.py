...
def scan_run_script(script: str) -> set[str]:
    ...
    require(
        not re.search(r"\b(?:invoke-expression|iex|eval)\b", lowered),
        "DYNAMIC_EXECUTION",
    ...
    words = set(re.findall(r"(?im)(?:^|[;&|]\s*)[&.]?\s*([a-z][a-z0-9_.-]*)", lowered))
    network = words & NETWORK_COMMANDS
    require(not network, "NETWORK_EXECUTION", f"run script invokes {sorted(network)}")
    packages = words & PACKAGE_COMMANDS
    require(not packages, "PACKAGE_EXECUTION", f"run script invokes {sorted(packages)}")
    for match in re.finditer(r"(?im)(?:^|[;&|]\s*)git(?:\.exe)?\s+([a-z-]+)", lowered):
        ...
        require(
            subcommand in {"rev-parse", "status", "remote"},
            "GIT_UNMODELED",
            f"run script invokes unmodeled git {subcommand}",
        )
    require(
        not re.search(r"(?im)(?:^|[;&|]\s*)(?:docker|podman)\b", lowered),
        "CONTAINER_EXECUTION",
        "run script invokes a container runtime",
    )
...
def validate_workflow_document(
    ...
    for event, config in triggers.items():
        ...
        require_exact_keys(config, {"types"}, "WORKFLOW_EVENT_CONFIG", event)
        require(isinstance(config["types"], list), "WORKFLOW_EVENT_TYPES", event)
        require(
            config["types"] == expected_types,
    ...
    require(
        permissions == workflow_spec["permissions"],
    ...
    require(
        "${{ secrets." not in strings.casefold(),
        "WORKFLOW_SECRET",
    ...
    for job_id, job in jobs.items():
        ...
        require(job["name"] == job_spec["name"], "WORKFLOW_JOB_NAME", job_id)
        require(job["runs-on"] == job_spec["runs_on"], "WORKFLOW_RUNNER", job_id)
        steps = job["steps"]
        ...
        for index, (step, step_spec) in enumerate(zip(steps, job_spec["steps"])):
            ...
            if kind == "action":
                ...
                require(step["env"] == step_spec["env"], "WORKFLOW_ENV", step["name"])
                ...
                if "${{ github.token }}" in _walk_strings(step["env"]):
                    require(
                        step_spec.get("github_token") is True,
                        "WORKFLOW_TOKEN",
...
def validate_approval_graph(graph: Mapping[str, Any]) -> None:
    ...
    require(graph["version"] == 2, "GRAPH_VERSION", "approval graph version must be 2")
    ...
    require(graph["approved_actions"] == APPROVED_ACTIONS, "GRAPH_ACTIONS", "action pins")
