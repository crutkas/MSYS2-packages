...
$fixtures = Get-Content (
    Join-Path $PSScriptRoot 'aarch64-pseudo-reloc-fixtures.json'
) -Raw | ConvertFrom-Json
$temp = Join-Path $env:TEMP (
    "pseudo-reloc-decoder-$([guid]::NewGuid().ToString('N'))")
        ...
        switch ($Fixture.format) {
            ...
            'v1' {
                foreach ($record in $Fixture.records) {
                    $writer.Write([uint32] $record.addend)
                    $writer.Write([uint32] $record.target_rva)
                }
            }
            'v2' {
            ...
            default {
                throw "unknown fixture format: $($Fixture.format)"
            }
        }
    }
    finally {
...
function Invoke-Expected {
    ...
    if ($result.result -ne $ExpectedResult) {
        throw "$Name result mismatch: expected $ExpectedResult, " +
            "got $($result.result)"
    }
...
try {
    foreach ($fixture in $fixtures) {
        ...
        if ($fixture.name -eq 'negative-64-21-12-21-12') {
            $violations = @(
                $result.policy_violations | ForEach-Object { [uint32] $_ })
            if (@(Compare-Object @(12, 21) $violations).Count -ne 0) {
                throw "Negative fixture violations mismatch: $violations"
            }

            if (-not $RealFixtureDirectory) {
                $bundled = Join-Path $PSScriptRoot 'fixtures\aarch64-pseudo-relocs'
                if (Test-Path -LiteralPath $bundled -PathType Container) {
                    $RealFixtureDirectory = $bundled
                }
            }
            $objdumpAvailable = (
                (Test-Path -LiteralPath $Objdump -PathType Leaf) -or
                $null -ne (
                    Get-Command $Objdump -CommandType Application `
                        -ErrorAction SilentlyContinue
                )
            )
            $nmAvailable = (
                (Test-Path -LiteralPath $Nm -PathType Leaf) -or
                $null -ne (
                    Get-Command $Nm -CommandType Application `
                        -ErrorAction SilentlyContinue
                )
            )
            if ($RealFixtureDirectory -and $objdumpAvailable -and $nmAvailable) {
                $realRoot = (Resolve-Path -LiteralPath $RealFixtureDirectory).Path
                $matrix = Get-Content (
                    Join-Path $realRoot 'matrix.json'
                ) -Raw | ConvertFrom-Json
                $externalNegative = Get-Content (
                    Join-Path $realRoot 'zlib-baseline-negative.json'
                ) -Raw | ConvertFrom-Json
                if (($externalNegative.flags -join ',') -ne '64,21,12,21,12' -or
                    ($externalNegative.policy_violations -join ',') -ne '12,21') {
                    throw 'zlib baseline negative proof contract changed'
                }
                foreach ($fixture in $matrix) {
                    $binary = Join-Path $realRoot $fixture.binary
                    $actualSha256 = (
                        Get-FileHash -Algorithm SHA256 -LiteralPath $binary
                    ).Hash.ToLowerInvariant()
                    if ($actualSha256 -ne $fixture.sha256) {
                        throw "$($fixture.binary) SHA-256 mismatch: $actualSha256"
                    }
                    Invoke-Expected `
                        -Name "real-$($fixture.binary)" `
                        -Arguments @(
                            '-PePath', $binary,
                            '-Objdump', $Objdump,
                            '-Nm', $Nm
                        ) `
                        -ExpectedExit $fixture.exit_code `
    ...
    if ($NegativePePath) {
        $result = Invoke-Expected `
            -Name 'negative-pe' `
            -Arguments @(
                '-PePath', $NegativePePath, '-Objdump', $Objdump, '-Nm', $Nm
            ) `
            -ExpectedExit 1 `
            -ExpectedResult 'fail' `
            -ExpectedFlags @(64, 21, 12, 21, 12)
        if (@(
            Compare-Object @(12, 21) @($result.policy_violations)
        ).Count -ne 0) {
    ...
    if ($PositivePePath) {
        $result = Invoke-Expected `
            -Name 'positive-pe' `
            -Arguments @(
                '-PePath', $PositivePePath, '-Objdump', $Objdump, '-Nm', $Nm
            ) `
            -ExpectedExit 0 `
            -ExpectedResult 'pass'
        if (@(
            $result.flags | Where-Object { $_ -in @(12, 21) }
        ).Count -ne 0) {
